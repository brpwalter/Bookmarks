...
/**
  ...
  */	
	public static int addBookmark(String bmName,String url,int rating, int accessright, int coId,int foId){
		System.out.println("Bookmark:addBookmark: "+foId+" "+bmName);
		Connection conn = null;
		int insertedId = -1;
		try{
			conn = ConnectionProvider.getConnection();
			...
			System.out.println("INSERT INTO vk_bookmark_folder11: "+insertedId+" "+foId);
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1,insertedId);
			pstmt.setInt(2,foId);

			pstmt.executeUpdate();
			System.out.println("INSERT INTO vk_bookmark_folder");
			
		} catch(Exception ex){ System.out.println("Bookmark:addBkm: "+ex);
		} finally{
			try{
				if(conn!=null) conn.close();
			} catch(Exception ex2){
  ...
  */	
	...
	*/		
	public static void increaseAccessfreq(int bmId){
		System.out.println("increaseAccessfreq "+bmId);
...
java.sql.Date d2=new java.sql.Date(utilDatum.getTime());
		try{
			conn = ConnectionProvider.getConnection();
			String sql = "UPDATE tb_bookmark SET frequency_of_access=frequency_of_access+1,date_of_access=? WHERE tb_bookmark_id=?";
			PreparedStatement pstmt = conn.prepareStatement(sql);
			pstmt.setDate(1,d2);
			pstmt.setInt(2,bmId);
			pstmt.executeUpdate();
			System.out.println ("increaseAccessfreq: erfolgreich akt.");
		} catch(Exception ex){
			System.out.println(mName + ex);
		} finally{
			try{
