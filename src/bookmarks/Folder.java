...
/**
  ...
  */
	public Folder(ResultSet rs) throws SQLException {
		...
		children=getList(foId);
		//children = getChildrenByUserId(0, foId);
  ...
  */
	public static List getList(){
		...
		try{
			...
			ResultSet rs = stmt.executeQuery("SELECT * FROM tb_folder order by tb_folder_id;");
			if(rs!=null){
  ...
  */
	public static List getList(int foId)throws NullPointerException{
		...
		//	conn.setAutoCommit(false);
			...
			if (conn == null)
				return fl;
			pstmt = conn.prepareStatement("SELECT * FROM tb_folder WHERE allocation=?");
			pstmt.setInt(1,foId);
			ResultSet rs = pstmt.executeQuery();
			if(rs!=null){
				while(rs.next()) fl.add(new Folder(rs));
...
//---------
public static List getList4(int foId)throws NullPointerException{
		String mName = CNAME + ".getList(foId): ";
		List fl = new LinkedList();
		Connection conn = null;
		try{
			conn = ConnectionProvider.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM tb_folder WHERE tb_folder_id=?");
			pstmt.setInt(1,foId);
			ResultSet rs = pstmt.executeQuery();
			if(rs!=null){
				while(rs.next()) fl.add(new Folder(rs));
...
/**
  ...
  */
	public static List getListByRootId(int rootId) throws NullPointerException{
		System.out.println("getListByRootId(int rootId)"+rootId);
		...
		try{
			...
			else System.out.println("rs ist null");	
		} catch(Exception ex1){
			System.out.println(mName + ex1);
			ex1.printStackTrace();
		} finally{
  ...
  */
	public static String openAll(int userId){
		...
		for(Iterator it1 = rootListe.iterator();it1.hasNext();){
			...
			rootId=fo1.getFoId();
			fiStr+=rootId+ "_";
			System.out.println("*****Folder: rootId: "+rootId);
			if (fl!=null )
			try { 
			...
			else System.out.println("f1 ist null");
...
/**
  ...
  */
	...
	public static void deleteFolderComplete(int foId){
		System.out.println("Folder:deleteFolderComplete"); 
